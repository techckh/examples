...
from examples.schemas import INPUT_SCHEMA, OUTPUT_SCHEMA
...
# Define a Global class an AWS Resource: Amazon S3 bucket.
class LambdaS3Class:
    """
...
def lambda_handler(event: APIGatewayProxyEvent, context: LambdaContext) -> Dict[str, Any]:
    ...
    # Explicitly pass the global resource to subsequent functions
...
def create_letter_in_s3(dynamo_db: LambdaDynamoDBClass,
    ...
    body = "OK"
    try:         
        # Use the passed environment class for AWS resource access - DynamoDB
        ...
        # Use the passed environment class for AWS resource access - S3
